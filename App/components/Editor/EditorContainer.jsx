...
import { changeContent as ideaChangeContent } from '../../slices/ideasSlice'
import { changeSectionContent, changeSectionName } from '../../slices/projectsSlice'
...
export const EditorContainer = (props) => {
    ...
    if (route.params.type === 'IDEA') {
        data = useSelector(state => state.Ideas.ideasData[route.params.uuid])
    ...
    if (route.params.type === 'PROJECT') {
        let temp = []
        data = useSelector(state => state.Projects.projectsData[route.params.uuid])
        let worldUuid = data.worldUuid
